...
class TS(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_stripes):
            ...
            S_logits_list.append(self.Sfc_list[i](local_feat))

        ######################################################
        # learn part-level feature for teacher
        ...
        for i in range(self.num_stripes):
            ...
            local_feat = self.local_conv_list[i](local_feat)
            local_feat = F.max_pool2d(local_feat, (stripe_h, feat.size(-1)))
            local_feat = self.local_conv_list2[i](local_feat)

            local_feat = local_feat.view(local_feat.size(0), -1)
            T_local_feat_list.append(local_feat)
            T_logits_list.append(self.Tfc_list[i](local_feat))

        return S_local_feat_list, S_logits_list, T_local_feat_list, T_logits_list
